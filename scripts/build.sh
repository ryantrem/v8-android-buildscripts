...
function buildArch()
{
  ...
  # Debug: list available GN args
  echo "=== Available GN args for ${output_dir} ==="
  gn args --list "${output_dir}" > /tmp/gn_args_list.txt 2>&1
  cat /tmp/gn_args_list.txt
  echo "=== End of GN args list ($(wc -l < /tmp/gn_args_list.txt) lines) ==="
